...
class EnhancedChannelAttention(nn.Module):
    """增强型通道注意力机制（融合平均池化和最大池化特征）"""

    def __init__(self, in_channel, reduction=16):
        """
        Args:
            in_channel: 输入特征图的通道数
            reduction: 通道缩减比例（自动防止过度缩减）
        """
        super().__init__()
        # 双路池化层：提取全局空间信息
        ...
        # 动态调整缩减比例（确保最小通道数不小于4）
        self.reduction = max(reduction, in_channel // 4)

        # 特征变换层：两层1x1卷积构成瓶颈结构
        self.fc = nn.Sequential(
            nn.Conv2d(in_channel, in_channel // self.reduction, 1, bias=False),  # 降维
            nn.ReLU(inplace=True),  # 保持内存连续
            nn.Conv2d(in_channel // self.reduction, in_channel, 1, bias=False),  # 升维
    ...
    def forward(self, x):
        ...
        return x * (avg_out + max_out)  # 特征加权（广播机制）


class EnhancedSpatialAttention(nn.Module):
    """多尺度空间注意力机制（空洞卷积金字塔结构）"""
    ...
    def __init__(self, in_channel):
        ...
        """
        super().__init__()
        # 创建并行卷积路径（不同空洞率的3x3卷积）
        self.conv_layers = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(in_channel, in_channel // 8, 1),  # 通道压缩
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channel // 8, 1, 3, padding=d, dilation=d),  # 空间特征提取
                nn.Sigmoid()  # 空间权重矩阵
            ) for d in [3, 6, 9]  # 空洞率配置（感受野依次增大）
        ])

    def forward(self, x):
        """多尺度特征融合（平均加权）"""
        spatial_weights = sum(layer(x) for layer in self.conv_layers) / 3  # 多尺度特征平均
        return x * spatial_weights  # 空间特征重校准


class DynamicFusion(nn.Module):
    """动态加权特征融合器（通道+空间注意力组合）"""
    ...
    def __init__(self, in_channel):
        ...
        super().__init__()
        # 注意力模块初始化
        self.channel_att = EnhancedChannelAttention(in_channel)  # 通道注意力
        self.spatial_att = EnhancedSpatialAttention(in_channel)  # 空间注意力

        # 可学习融合权重（初始化为0.5±0.1的正态分布）
        self.weight = nn.Parameter(torch.randn(2) * 0.1 + 0.5)  # 两个权重参数

    def forward(self, x):
        """动态特征融合（带残差连接）"""
        channel_feat = self.channel_att(x)  # 通道校准后的特征
        spatial_feat = self.spatial_att(x)  # 空间校准后的特征

        # 自适应权重计算（Softmax归一化）
        weights = F.softmax(self.weight, dim=0)  # 权重和为1
...
class MDFA(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, rates=[1, 3, 6, 9], bn_mom=0.1):
        ...
        # 动态分支权重（可学习参数）
        self.branch_weights = nn.Parameter(torch.ones(len(rates) + 1))  # 分支数+全局分支

        # 全局上下文分支（压缩-激励结构）
        self.global_branch = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),  # 全局平均池化
            nn.Conv2d(dim_in, dim_out, 1),  # 特征变换
            nn.Hardsigmoid()  # 非线性激活（部署友好）
        )
        ...
        self.output_conv = nn.Sequential(
            nn.Conv2d(dim_out, dim_out, 3, padding=1, bias=False),  # 3x3卷积细化特征
            nn.BatchNorm2d(dim_out, momentum=bn_mom),  # 批量归一化
    ...
    def _build_dilation_branch(self, in_c, out_c, dilation):
        ...
        return nn.Sequential(
            nn.Conv2d(in_c, out_c, 3,
                      padding=dilation, dilation=dilation, bias=False),  # 空洞卷积
            nn.BatchNorm2d(out_c),  # 批量归一化
            nn.ReLU(inplace=True)  # 激活函数
        )
    ...
    def forward(self, x):
        """前向传播（多尺度特征融合流程）"""
        # 处理多尺度分支特征
        branch_features = [branch(x) for branch in self.branches]  # 各分支特征列表

        # 处理全局特征
        global_feat = self.global_branch(x)  # 全局上下文信息
        global_feat = F.interpolate(global_feat, x.shape[2:],  # 上采样到原始尺寸
                                    mode='bilinear', align_corners=False)

        # 特征合并策略（维度说明：B=批量, C=通道, H=高度, W=宽度）
        all_features = torch.stack(branch_features + [global_feat], dim=1)  # [B, N+1, C, H, W]

        # 动态权重计算（维度变换以适应广播机制）
        weights = F.softmax(self.branch_weights, dim=0)  # 分支权重归一化
        weights = weights.view(1, -1, 1, 1, 1)  # 重塑为[1, N+1, 1, 1, 1]

        # 加权融合（逐分支相乘后求和）
        fused_features = (all_features * weights).sum(dim=1)  # [B, C, H, W]

        # 特征精炼与输出
        refined_features = self.feature_fusion(fused_features)  # 融合通道+空间注意力
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 改进1: 可学习的加权融合
        weights = torch.softmax(self.weights, dim=0)  # 归一化权重
...
class MECS(nn.Module):
    def __init__(self, in_channels, out_channels, channel_attention_reduce=4):
        ...
        self.depth_convs = nn.ModuleList([
            ...
            nn.Conv2d(in_channels, in_channels, 3, padding=2, dilation=2, groups=in_channels),  # 新增膨胀卷积
        ])
